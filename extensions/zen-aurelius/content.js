...
function findElement(selector) {
  ...
  const looksLikeCss = /^[#.\[a-zA-Z*]/.test(s) && !s.includes(' ') ||
    /^[#.\[]/.test(s) ||
    /\[.*\]/.test(s);
  ...
  for (const el of all) {
    ...
    if (name.includes(needle) || id.includes(needle) || title.includes(needle)) return el;
  }
...
function extractPageContent() {
  ...
  if (clone) {
    const garbage = clone.querySelectorAll('script, style, svg, noscript, iframe, canvas');
...
function findInteractiveElements(filterQuery) {
  ...
  for (const el of candidates) {
    ...
    const parts = [`<${tag}${type ? ` type="${type}"` : ''}${role ? ` role="${role}"` : ''}>`];
...
function handleHover(params) {
  ...
  const tag  = el.tagName.toLowerCase();
  const hint = el.getAttribute('aria-label') || el.textContent?.trim().slice(0, 40) || '';
...
function handleSelectOption(params) {
  ...
  if (el.tagName.toLowerCase() !== 'select') {
    throw new Error(`Element "${params.selector}" is not a <select> (found <${el.tagName.toLowerCase()}>)`);
  }
  ...
  for (const option of el.options) {
    ...
    const matchLabel = label != null && option.text.trim().toLowerCase().includes(label.toLowerCase());
  ...
  try {
    switch (action) {
      ...
      case 'type': {
        ...
        const tag = el.tagName.toLowerCase();
        const masked = params.secret ? '•'.repeat(text.length) : `"${text.slice(0, 50)}${text.length > 50 ? '…' : ''}"`;
